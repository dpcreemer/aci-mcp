class Interface(object):
  ...
  def id(self, interface):
    ...
    d = self.__node.query('l1PhysIf', filter=f'eq(l1PhysIf.id, "{interface}"').run()
    if d.count != 1:
  ...
  def admin_state(self):
    return self.node.query(self.dn).run().value('adminSt')
  ...
  @property
  def state(self):
    return self.node.query(f'{self.dn}/phys').run().value('operSt')

  @property
  def speed(self):
    return self.node.query(self.dn).run().value('speed')

  @property
  def oper_speed(self):
    return self.node.query(f'{self.dn}/phys').run().value('operSpeed')
  ...
  @property
  def crc_errors(self):
    return int(self.node.query(f'{self.dn}/dbgEtherStats').run().value('cRCAlignErrors'))

  @property
  def packets(self):
    return int(self.node.query(f'{self.dn}/dbgEtherStats').run().value('pkts'))

  @property
  def packets_in(self):
    packet_data = self.node.query(f'{self.dn}/dbgIfIn').run()
    return int(packet_data.value('ucastPkts')) + int(packet_data.value('nUcastPkts'))

  @property
  def packets_out(self):
    packet_data = self.node.query(f'{self.dn}/dbgIfOut').run()
    return int(packet_data.value('ucastPkts')) + int(packet_data.value('nUcastPkts'))

  @property
  def input_errors(self):
    return int(self.node.query(f'{self.dn}/dbgIfIn').run().value('errors'))

  @property
  def output_errors(self):
    return int(self.node.query(f'{self.dn}/dbgIfOut').run().value('errors'))

  @property
  def packet_data(self):
    data = self.node.query(self.dn, target='children', target_class='rmonEtherStats,rmonIfIn,rmonIfOut').run()
