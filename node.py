...
class Node(object):
	...
	@property
	def interfaces(self):
		return self.qr('l1PhysIf').data.attribute('id')
	...
	def __init_values(self):
		d = self.qr('topSystem', filter=f'eq(topSystem.oobMgmtAddr, "{self.ip.ip}")').output
	...
	def __find_name(self):
		return self.qr('topSystem', filter=f'eq(topSystem.id, "{self.id}")').output.value('name')
	...
	# ssh to current node and log to a local log file
	def ssh(self):
		ts = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
		path = f"{os.getcwd()}/ssh_logs"
		if not os.path.exists(path):
			os.mkdir(path)
		logfile = f"{path}/{self.address}_{ts}.log"
		os.system('ssh ' + self.username + "@" + self.address + " | tee -a " + logfile)
	...
	def qr(self, path=None, filter=None, target=None, target_class=None, include=None, subtree=None, subtree_class=None,
						subtree_filter=None, subtree_include=None, order=None):
		query = Query(self, path=path, filter=filter, target=target, target_class=target_class, include=include,
									subtree=subtree, subtree_class=subtree_class, subtree_filter=subtree_filter,
									subtree_include=subtree_include, order=order)
		query.run()
		return query
	...
	def exists(self, dn):
		...
		q = self.qr(dn, include='naming-only')
		if q.output.count == '0':
			return False
	...
	def get_class(self, dn):
		...
		q = self.qr('mo/%s.json' % dn)
		return list(q.output.imdata[0].keys())[0]

	def remove_object(self, dn):
		js = self.qr(dn, include='naming').output.imdata[0]
	...
	def cdp_neighbors(self, ifc=None):
		if ifc is None:
			nbrs = self.qr('cdpAdjEp').data
		else:
			ifc = Interface(self, ifc)
			nbrs = self.qr(f'{self.dn}/cdp/inst/if-[{ifc.id}]', target='subtree', target_class='cdpAdjEp').data
	...
	def lldp_neighbors(self, ifc=None):
		if ifc is None:
			nbrs = self.qr('lldpAdjEp').data
		else:
			ifc = Interface(self, ifc)
			nbrs = self.qr(f'{self.dn}/lldp/inst/if-[{ifc.id}]', target='subtree', target_class='lldpAdjEp').data
